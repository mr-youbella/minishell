...
/*   Updated: 2025/06/22 21:58:24 by youbella         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static t_redirections_output	*last_node(t_redirections_output *lst)
{
	if (!lst)
		return (NULL);
	while (lst->next)
...
static void	add_node_in_back(t_redirections_output **lst, t_redirections_output *new)
{
	if (!new)
		return ;
	if (!*lst)
...
static t_redirections_output	*craete_new_node(char *redirect_type, char *file)
{
	t_redirections_output	*node;

	node = malloc(sizeof(t_redirections_output));
	if (!node)
		return (NULL);
...
t_redirections_output	*add_redirections_out_and_filename_in_list(char *str)
{
	...
	t_redirections_output	*list;
	t_redirections_output	*new_node;
	char					*redirec_type;
	char					*file_name;
